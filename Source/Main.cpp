...
DeckComponent::DeckComponent(const juce::String& deckName)
{
    ...
    {
        onGain(static_cast<float>(volumeSlider.getValue() / 100.0));
    };
...
void DeckComponent::setPlayback(double position, double length, float level)
{
    ...
    const auto progress = safeLength > 0.0
        ? juce::jlimit(0.0, 1.0, position / safeLength)
        : 0.0;

    positionSlider.setValue(progress, juce::dontSendNotification);
    timeLabel.setText(juce::String::formatted(
        "%02d:%02d / %02d:%02d",
        static_cast<int>(position) / 60,
        static_cast<int>(position) % 60,
        static_cast<int>(safeLength) / 60,
        static_cast<int>(safeLength) % 60),
...
void DeckComponent::paint(juce::Graphics& g)
{
    ...
    const auto meter = area.withTrimmedLeft(18.0f).withTrimmedRight(18.0f)
                           .withY(92.0f).withHeight(8.0f);
    g.setColour(juce::Colours::black.withAlpha(0.35f));
    g.fillRoundedRectangle(meter, 4.0f);

    g.setColour(juce::Colour(0xff58d68d));
    g.fillRoundedRectangle(meter.withWidth(meter.getWidth() * outputLevel), 4.0f);
