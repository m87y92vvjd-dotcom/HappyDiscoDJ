...
MixerComponent::MixerComponent()
{
    ...
    crossfader.onValueChange = [this] { onCrossfade(static_cast<float>(crossfader.getValue())); };
    ...
    masterGain.onValueChange = [this] { onMasterGain(static_cast<float>(masterGain.getValue())); };
...
void MixerComponent::paint(juce::Graphics& g)
{
    ...
    g.fillRoundedRectangle(getLocalBounds().toFloat().withTrimmedTop(6.0f).withTrimmedBottom(6.0f).withTrimmedLeft(getWidth() * 0.42f).withTrimmedRight(getWidth() * 0.42f), 3.0f);
}
